<?php
/*<rt/form-data"加密的POST类型(一般上传文件或发贴使用这个加密).
	用途: ......
	编写: chenall
	时间: 2012-11-27
	版本: 1.1
...
if ($debug)
{
	if (!file_exists('debug/'))
		$mkdir_ret = mkdir('debug', 0777);
	$debug = fopen("debug/123.log",'a');//.$_SERVER['SERVER_NAME'].'__'.date('_ymdHis_',time()).'__'.$_SERVER['REMOTE_ADDR'].".log",'a');
